...
function NestedMultiselect(props: NestedMultiselectProps) {
  ...
  // Updated useOutsideClick hook to close dropdown properly
  useOutsideClick(closePopup, rootRef)
  ...
  const pop = () => {
    ...
    find({ value: "", label: "", children: options })
  ...
  // Ensure when the popup closes, the activeOption resets to root level
  useEffect(() => {
    if (!isOpen) {
      setActiveOption({
        value: "",
  ...
  // Call onSelect whenever selected changes
  useEffect(() => {
  ...
  return (
    <div ref={rootRef} className="relative h-[40px]">
      <Input
        isOpen={isOpen}
        openPopup={openPopup}
